...
// Dispatch search changes directly to the DOM
const dispatchSearchChange = (query: string) => {
  if (typeof window !== 'undefined') {
    window.dispatchEvent(new CustomEvent('filterChange', { detail: { searchQuery: query } }));
  }
};
...
export const SearchBar: React.FC<SearchBarProps> = ({
  ...
  const handleSelect = (result: SearchResult) => {
    setQuery('');
    setIsOpen(false);
    // Navigate directly to the cut page
    if (typeof window !== 'undefined') {
      window.location.href = `/cuts/${result.cut.id}`;
    }
  };
